...
```golang
import udmclient "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
```golang
ctx := context.WithValue(context.Background(), udmclient.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), udmclient.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), udmclient.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), udmclient.ContextOperationServerVariables, map[string]map[string]string{
