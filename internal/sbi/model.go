...
type SbiConfig struct {
	...
	Udm struct {
		IpAddr            string `envconfig:"UDM_IP_ADDR"`
		SubRoute          string `envconfig:"UDM_SUBSCR_ROUTE"`
		ApiRoute          string `envconfig:"UDM_API_ROUTE"`
		NotifCorrId       string `envconfig:"UDM_NOTIFY_CORRELATION_ID"`
		NotifId           string `envconfig:"UDM_NOTIFICATION_ID"`
		NorifForwardRoute string `envconfig:"UDM_NOTIFICATION_FORWARD_ROUTE"`
	}
	Database struct {
		...
		CollectionUdmName string `envconfig:"MONGODB_COLLECTION_NAME_UDM"`
	}
	Server struct {
...
type Reachability struct {
	UeReachability amf_client.UeReachabilityAnyOf
	TimeStamp      int64
}
...
type CommFailure struct {
	...
	RanReleaseCode amf_client.NgApCause
	TimeStamp      int64
}
