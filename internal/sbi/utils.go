...
import (
	...
	udm_client "oai-nwdaf-sbi/internal/udemclient"
...
func InitConfig() {
	...
	// Subscribe to all event notifications from UDM
	udmEventSubscription(
		config.Server.NotifUri+config.Udm.ApiRoute,
		config.Udm.NotifId,
	)
}
...
func udmEventSubscription(
	udmEventNotifyUri string,
	udmNotifyCorrelationId string,
	udmNfId string,
) {
	// Store all AMF event types
	var udmEvents []udm_client.UdmEvent
	for _, amfEventTypeAnyOf := range amf_client.AllowedAmfEventTypeAnyOfEnumValues {
		amfEvents = append(amfEvents, *amf_client.NewAmfEvent(amfEventTypeAnyOf))
	}
	// Subscribe to all AMF event types
